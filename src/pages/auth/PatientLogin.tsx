...
const PatientLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const profileRole = profile?.role ?? 'patient';
      if (profileRole !== 'patient') {
        await supabase.auth.signOut();
        setError('This is not a patient account. Please use Doctor Login.');
        return;
      }
    ...
    <div className="min-h-screen w-full bg-fitness-background text-white flex items-center justify-center p-4">
      ...
      <Card className="w-full max-w-md bg-fitness-background/80 border border-fitness-primary/20 relative z-10">
        ...
        <CardFooter className="flex flex-col space-y-2">
          ...
          <div className="text-sm text-center text-gray-400">
            Are you a doctor?{' '}
            <Link to="/signin/doctor" className="text-fitness-accent hover:underline">
              Doctor Login
            </Link>
          </div>
        </CardFooter>
